...
class Alumno {
    ...
    public static function updateAlumno($data)
    {
        ...
        if(empty($password)){
            $db = new Database();
            $connection = $db->connect();
            $res = $connection->query("UPDATE usuarios  set matricula = '$matricula', nombre = '$nombre', apellido = '$apellido', correo = '$correo', direccion = '$direccion', fecha_nacimiento = '$fecha_nacimiento'  where id_usuario = $id_usuario"); 
        }else {
            $hash = password_hash($password, PASSWORD_DEFAULT);
            $db = new Database();
            $connection = $db->connect();
            $res = $connection->query("UPDATE usuarios  set  matricula = '$matricula', nombre = '$nombre', apellido = '$apellido', correo = '$correo', password = '$hash', direccion = '$direccion', fecha_nacimiento = '$fecha_nacimiento'  where id_usuario = $id_usuario");  
        }  
        if ($res) {
            return true;
        } else {
